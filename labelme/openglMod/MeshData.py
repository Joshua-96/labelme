...
class MeshData(object):
    ...
    def __init__(self, vertexes=None, faces=None, edges=None, vertexColors=None, faceColors=None, meshWidth=None):
        """
        ==============  =====================================================
        **Arguments:**
    ...
    def faceNormals(self, indexed=None):
        ...
        if self._faceNormals is None:
            v = self.vertexes(indexed='faces').astype(np.float32)
    ...
    def vertexNormals(self, indexed=None):
        ...
        if self._vertexNormals is None:
            ...
            sumed_norms = norm_tuple.sum(axis=1)
    ...
    def get_normTuple(self, faceNorms):
        if self._vertexFaces is None:
            normTuple = np.empty(
                (self._vertexes.shape[0], 6, 3),
                dtype=np.float32)
            normTuple.fill(np.nan)
            # vertexCount = [0 for i in range(len(self.vertexes()))]
            vertexCount = np.zeros((len(self.vertexes())),dtype=np.uint8)
            step = self.meshWidth - 1
            for i in range(0,self._faces.shape[0],step):
                for j in range(3):
                    rg = np.arange(i, np.min((self._faces.shape[0], i+step)))
                    faces = self._faces[rg]
                    normTuple[faces[:,j], vertexCount[faces[:,j]]] = faceNorms[rg]
